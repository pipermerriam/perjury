...
class BaseGenerator(object):
    ...
    def __call__(self, *args, **kwargs):
        """
        Delegates to :func:`BaseGenerator.next`
        """
    ...
    def initialize_hashes(self):
        """
        Allows for enforcing uniquness beyond the values generated by the
        generator class itself.

        Assuming you were using a generator to populate usernames in a
        database, you could initialize :attr:`BaseGenerator.hashes` with a list
        of all of the currint usernames your database to ensure that new values
        were unique.
        """
        self.hashes = set()
    ...
    def generator(self):
        """
        This method must be implemented on subclasses of ``BaseGenerator`` and
        should return an iterator with a few caveats.

        Due to the way uniqueness is handled, when
        ...
        'infinite generator' should be returned.
        """
        raise NotImplementedError('Genertator classes must implement their own `generator` method.')
    ...
    def outer_generator(self):
        while True:
            ...
            while True:
                ...
                retval = self.cached_generator().next()
                hash = self.hash_function(retval)
    ...
    def reset(self):
        ...
        self._outer_generator = self.outer_generator()
    ...
    def next(self):
        ...
        """
        return self._outer_generator.next()
